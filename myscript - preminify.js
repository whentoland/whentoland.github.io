function buttonClick()
{
	if (document.getElementById('theButton').classList.contains('button')) {
		...
		var battAmps = document.getElementById("battAmps").options[document.getElementById("battAmps").selectedIndex].value;
		var wattsDrain = document.getElementById("wattsDrain").options[document.getElementById("wattsDrain").selectedIndex].value;
		var motorPower = document.getElementById("motorPower").options[document.getElementById("motorPower").selectedIndex].value;
		var throttle = document.getElementById("throttle").options[document.getElementById("throttle").selectedIndex].value;
		var theFinalDrain;
		
		if (wattsDrain!="unselected") theFinalDrain=wattsDrain; else theFinalDrain=motorWattsCalc(motorPower,throttle);
		
		var ampsToLand=battAmps*0.7;
		var flightTime=battWattsCalc(battCells,battAmps)*0.7/theFinalDrain*60;
		console.log("thefinaldrain",theFinalDrain);
		var voltTable=[];
		...
		galabel = battCells + ';' + battAmps + ';' + wattsDrain + ';'+ motorPower + ';'+ throttle +';'+counter;
...
function inputSelect(whichone)
{
	...
	if (whichone=="wattsDrain") {
		document.getElementById('motorPower').value = "unselected";
		document.getElementById('throttle').value = "unselected";
	}
	if ((whichone=="motorPower")||(whichone=="throttle")) document.getElementById('wattsDrain').value = "unselected";
	...
	if ((battCells!="unselected")&&(battAmps!="unselected")&&((wattsDrain!="unselected")||((motorPower!="unselected")&&(throttle!="unselected")))) {
...
function battWattsCalc(cells, milliamps)
{
	return milliamps/1000*4.2*cells
}
